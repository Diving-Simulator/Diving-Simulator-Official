...
public class SubmarineController : MonoBehaviour
{
    ...
    [Header("Som")]
    public AudioSource motorSubmarino;
    ...
    {
        if (motorSubmarino != null)
        {
            
            float velocidadeAtualSom = Mathf.Clamp01(velocidadeAtual);

            float volumeAtual = Mathf.Lerp(volumeMin, volumeMax, velocidadeAtualSom);
            motorSubmarino.volume = volumeAtual;
        }
        else
        {
            Debug.Log("O audio source do motor não foi detectado");
        }
    }
}